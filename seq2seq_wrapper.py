...
class Seq2Seq(object):
    ...
    def __init__(self, xseq_len, yseq_len,
        ...
        #  and testing model
        with tf.variable_scope(name_or_scope="decoder",reuse=tf.AUTO_REUSE) as scope:
    ...
    def train(self, train_set, valid_set, sess=None):
        ...
        for i in range(self.epochs):
            print('\nietration {} begin'.format(i))
            try:
                self.train_batch(sess, train_set)
                l,_,_,_ = self.eval_step(sess,valid_set)
                print('val loss: {}'.format(l))
